...
const sayHelloButton = document.querySelector('#say-hello-button')  //.class #id element by itself just ()
...
sayHelloButton.addEventListener('mouseover', originalColor)
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals')
        .then((res) => {for(let i=0; i < res.data.length; i ++){
            ...
            document.querySelector('body').appendChild(newName)
          }
...
const coolFunction = () => {
    axios.get(`http://localhost:3000/query-test/?football=chargers`)
    .then(res =>{
...
document.getElementById('query-button').addEventListener('click', coolFunction )
